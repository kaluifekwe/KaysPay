...
serve(async (req: Request) => {
  ...
  if (!(await isServiceEnabled(supabase, "nin_modification"))) {
    return json({
      success: false,
      error:
        "NIN Modification is temporarily unavailable. Please check back later.",
    }, 503);
  }
  ...
  const priceKobo = await getServicePriceKobo(supabase, "nin_modification", MODIFICATION_PRICE_KOBO);
