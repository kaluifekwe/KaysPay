...
import { submitNinModification, isNinBvnConfigured, NinBvnError, NinModificationType } from "../_shared/ninbvn-client.ts";
...
serve(async (req: Request) => {
  ...
  if (!isNinBvnConfigured()) return json({ error: "NIN modification not configured" }, 500);
  ...
  if (!TX_TYPE[modType]) return json({ success: false, error: "Invalid modification type" }, 400);
  ...
  if (!surname || !firstname) return json({ success: false, error: "Current surname and first name are required" }, 400);
  ...
  if (modType === "name") {
    ...
    if (!/^0\d{10}$/.test(phoneNumber)) return json({ success: false, error: "Enter a valid phone number on file" }, 400);
    if (!newSurname || !newFirstname) return json({ success: false, error: "Enter the corrected name" }, 400);
    providerFields = { ...providerFields, phone_number: phoneNumber, new_surname: newSurname, new_firstname: newFirstname };
  } else if (modType === "phone") {
    const newPhoneNumber = str(body?.new_phone_number);
    if (!/^0\d{10}$/.test(newPhoneNumber)) return json({ success: false, error: "Enter a valid new phone number" }, 400);
    providerFields = { ...providerFields, middlename: str(body?.middlename), new_phone_number: newPhoneNumber };
  } else {
    const phoneNumber = str(body?.phone_number);
    const newAddress = str(body?.new_address);
    if (!/^0\d{10}$/.test(phoneNumber)) return json({ success: false, error: "Enter a valid phone number on file" }, 400);
    if (!newAddress) return json({ success: false, error: "Enter the corrected address" }, 400);
    providerFields = { ...providerFields, phone_number: phoneNumber, new_address: newAddress };
  }
  ...
  // resolves it.
  try {
    const { status, data } = await submitNinModification(modType, providerFields);
    const referenceId = data?.reference_id ?? data?.data?.reference_id ?? data?.order_id ?? data?.data?.order_id;
    ...
    if (status >= 400 || !referenceId) {
      ...
      return json({ success: false, error: data?.message || "Could not submit this request. You were not charged." });
    }

    await supabase
      .from("transactions")
      .update({ metadata: { service: txType, ...providerFields, reference_id: referenceId } })
      .eq("id", txId);
    ...
    return json({
      ...
      pending: true,
      transaction_id: txId,
      message: "Request submitted. This usually takes 24-48 hours — check Transaction History for the result.",
    });
  } catch (e) {
    const isConfigError = e instanceof NinBvnError;
    await supabase.rpc("refund_service_transaction", {
      p_tx_id: txId,
      p_reason: isConfigError ? `ninbvn_config: ${e.message}` : "provider_unreachable",
    });
    return json({
      success: false,
      error: isConfigError ? "Provider not configured. Please try again later." : "Network error. Please try again. You were not charged.",
    });
  }
});
