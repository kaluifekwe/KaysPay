...
import { setStatus, isGrizzlySMSConfigured } from "../_shared/grizzlysms-client.ts";
...
// Cancels a rented number that never received its code. Refunding the
// customer's wallet is best-effort, not guaranteed: GrizzlySMS's own
// refund behavior on cancel isn't documented with certainty, so this only
// refunds when their response doesn't look like an outright rejection.
serve(async (req: Request) => {
  ...
  try {
    const response = await setStatus(activationId, -1);
    const rejected = /^BAD_|ERROR/i.test(response.trim());

    if (rejected) {
      return json({
        success: true,
        refunded: false,
        message: "Cancellation request sent, but the provider did not confirm a refund. This purchase may not be refundable.",
      });
    }
