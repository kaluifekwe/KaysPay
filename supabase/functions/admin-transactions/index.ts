...
serve(async (req) => {
  ...
  // NIN/BVN sub-types under one "Identity Verification" filter option) �?  // comma-separated, each checked against the same allowlist as before.
  ...
  for (const row of rows as unknown as {
    ...
    // crypto row's amount fell back to a bare "₦0" no matter what the
