...
function extractRecord(data: any): any {
  ...
  for (const c of candidates) {
    if (
      ...
      // Prembly returns DD-MM-YYYY; CheckMyNINBVN already returns YYYY-MM-DD.
      // normalizeDob was previously only applied inside buildMatchReport's
      // internal comparison, never to the record actually returned to the
      // client -- so a Prembly-sourced birthdate reached the NIN slip
      // unconverted and failed formatDobDisplay's YYYY-MM-DD regex there,
      // printing the raw "DD-MM-YYYY" string instead of "DD MON YYYY".
      if (c.birthdate) c.birthdate = normalizeDob(c.birthdate);
      return c;
    }
  }
  return undefined;
