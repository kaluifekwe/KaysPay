...
// Betting wallet funding — routed to VTUAfrica's `/betpay` endpoint (VTU.ng
// has no working betting integration: it has a `/betting` route but no
// discoverable service_id list, and every guessed slug was rejected live).
// Bounds: ₦100 .. ₦100,000. `id` is the exact `service` code VTUAfrica's own
// docs list (confirmed live at vtuafrica.com.ng/api/betting.php).
...
export const VALID_BETTING_IDS = BETTING_PROVIDERS.map((p) => p.id);
