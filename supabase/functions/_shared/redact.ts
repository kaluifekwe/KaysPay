...
export function redactSecrets(input: unknown): string {
  let s = typeof input === "string" ? input : String((input as { message?: unknown })?.message ?? input ?? "");
  // apikey / token / secret / password carried as a query param
  s = s.replace(/([?&](?:apikey|api_key|token|secret|password|pass|key)=)[^&\s)"']+/gi, "$1***");
  ...
  s = s.replace(/(\b(?:secret(?:_?hash|_?key)?|api[_-]?key)\b["']?\s*[:=]\s*["']?)[^\s"',}]+/gi, "$1***");
  return s;
}
