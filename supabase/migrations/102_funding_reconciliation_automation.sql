-- Run provider-led funding recovery every five minutes and extend the existing
-- read-only financial monitor with funding-specific failure signals.
...
RETURNS JSONB LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT jsonb_build_object(
    ...
    'funding_unresolved',(SELECT count(*) FROM public.funding_events WHERE status IN ('received','unmatched','error') AND last_seen_at<now()-INTERVAL '5 minutes'),
...
SELECT cron.schedule('funding-reconcile','*/5 * * * *',$$
