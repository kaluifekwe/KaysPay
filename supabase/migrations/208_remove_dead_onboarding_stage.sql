-- The "Onboarding started" funnel stage counted a slideshow screen that has
-- never actually been reachable from any navigation path in the app (a real
-- bug, confirmed by grepping the whole client for any navigate('Onboarding')
-- call -- there is none, and never has been per git history). It always
-- reads 0, sitting between two real, non-zero steps, which looks broken and
-- is confusing to read. The screen itself is being removed from the client;
-- this drops the corresponding phantom stage from the report so the funnel
-- only shows steps that can actually happen. Body otherwise byte-identical
-- to migration 207's version.
