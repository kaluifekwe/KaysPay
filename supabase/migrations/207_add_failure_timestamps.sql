-- "Top failure reasons" only ever showed aggregate counts (affected
-- installations, attempts) with no way to tell whether a row happened just
-- now or three weeks ago without opening individual events. Adds
-- first_occurred_at/last_occurred_at per (event_type, failure_code) row.
-- Body otherwise byte-identical to migration 190's version.
