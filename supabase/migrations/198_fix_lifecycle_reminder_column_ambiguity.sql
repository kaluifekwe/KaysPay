-- migration 197's claim_due_lifecycle_reminders failed at runtime with
-- "column reference attempt_number is ambiguous" -- caught by a read-only
-- verification call before any real reminder was ever sent. Cause: the
-- function's RETURNS TABLE(...) implicitly declares attempt_number as a
-- plpgsql OUT-parameter variable, which collides with the real
-- lifecycle_reminders.attempt_number column (referenced bare in the
-- ON CONFLICT target and the INSERT column list). #variable_conflict
-- use_column tells plpgsql to prefer the table column in exactly this case,
-- the standard fix for this class of ambiguity. Function body is otherwise
-- byte-identical to migration 197's version.
