-- Two fixes to the onboarding funnel report, both cosmetic/reporting-only
-- (no client or money-movement impact):
--
-- 1. "PIN set" is displayed after "Email verified" in the stages array, but
--    the PIN is actually saved during the Registration screen itself, before
--    the email-verification gate (see AppNavigator.tsx's RequireEmailVerify
--    onComplete comment). That made "PIN set" look like it exceeded its
--    "prior" step whenever a user hadn't verified yet -- not a real
--    regression, just the two steps listed in the wrong relative order.
--
-- 2. home_viewed (and everything after it) counted ANY installation that
--    reached the Home screen, including a returning user who already has an
--    account and is just logging in on a fresh install (new phone, reinstall,
--    cleared app data). That install never touches registration/PIN/email
--    verification at all, so "reached home" ballooned past every real
--    signup step. Now those stages only count installations that actually
--    started registration on this device; returning-login installations are
--    reported separately instead of silently inflating the funnel.
...
WITH cohort AS (
  ...
  SELECT count(*) activated,
    ...
    -- Scoped to registration_started_at IS NOT NULL from here down -- see
    -- comment above. Without this, a returning-user login on a fresh
    -- install counts toward "reached home" (and beyond) without ever
    -- appearing in any of the signup-specific steps above.
    count(*) FILTER(WHERE home_viewed_at IS NOT NULL AND registration_started_at IS NOT NULL) home,
    count(*) FILTER(WHERE kyc_started_at IS NOT NULL AND registration_started_at IS NOT NULL) kyc_started,
    count(*) FILTER(WHERE kyc_completed_at IS NOT NULL AND registration_started_at IS NOT NULL) kyc_completed,
    count(*) FILTER(WHERE funding_started_at IS NOT NULL AND registration_started_at IS NOT NULL) funding_started,
    count(*) FILTER(WHERE first_funding_at IS NOT NULL AND registration_started_at IS NOT NULL) funded,
    count(*) FILTER(WHERE first_purchase_at IS NOT NULL AND registration_started_at IS NOT NULL) purchased,
    count(*) FILTER(WHERE registration_started_at IS NULL AND home_viewed_at IS NOT NULL) returning_logins
  FROM cohort
), failures AS (
  ...
  FROM (
    SELECT e.event_type,e.failure_code,
      ...
      min(e.occurred_at) first_occurred_at,max(e.occurred_at) last_occurred_at,
      CASE e.failure_code
...
SELECT jsonb_build_object(
  ...
  'returning_login_installations',t.returning_logins,
  ...
  'stages',jsonb_build_array(
    ...
    jsonb_build_object('key','pin_setup','count',t.pin_setup),jsonb_build_object('key','email_verified','count',t.verified),
