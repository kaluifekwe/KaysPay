-- Persist sanitized evidence for every provider check performed before an
-- admin refund. Merging one namespaced object preserves existing metadata.
...
DECLARE
  v_updated INTEGER := 0;
BEGIN
  IF p_outcome NOT IN ('success', 'failed', 'unknown') THEN
